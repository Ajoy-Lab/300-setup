...
%     assert(res.ErrCode == 0);
    ...
    lengths = [60e-6 60e-6];
    phases = [0 90];
    mods = [3 3]; %0 = square, 1=gauss, 2=sech, 3=hermite 
...
%                 tof = -1000*cmdBytes(2);
                ...
                %inst.SendScpi(sprintf(':DIG:DDC:CFR2 0.0'))
...
%         ## get the length of the segment
        mkrsegment_length = length(dacWave)/4;
        
        % ## make a new segment
        onLength_points = onLength*sampleRateDAC;
        buffer_start = buffer*sampleRateDAC;
        
%         ## make the marker
        myMkr2((buffer_start+1) : (buffer_start+onLength_points)) = 1;% mkr_vector_on;
        myMkr2 = uint8(myMkr2);
    end
...
segMat = cell(3, numSegs);
   ...
   segMat{3,2*z} = linspace(0, DClenreal/sampleRateDAC, DClenreal);
       ...
       %%% make pulse segment %%% 
   [segMat{1,2*z+1}, segMat{2,2*z+1}] = makeSqPulse(frequencies(z), lengthsPts(z),  amps(z), phases(z), mods(z));
