...
%                 rc = inst.SetAdcExternTrigDelay(delay+0.000001+33.8e-6+33.8e-6 - 12e-6);
                ...
                rc = inst.SendScpi(sprintf(':DIG:TRIG:DEL:EXT %g', 8.5e-6)); %4.1
                assert(rc.ErrCode == 0);
...
%                 this section should normally be used:
...
% %                 only use the following section if running manually:
%                 awg_center_freq = 3.775e7;
%                 awg_bw_freq = 24e4;
%                 awg_amp = 1.2;
%                 sweep_freq = 750;
%                 sweep_sigma = 0.1;
...
% inst.SendScpi(':TRIG:ACTIVE:SEL INT'); 
...
%                 use this section unless you want to run manually:
                res = inst.SendScpi(':SOUR:FUNC:MODE TASK');
                assert(res.ErrCode == 0);
                
% %                 use the following section to run manually:
...
%                 assert(res.ErrCode == 0);
%                 % Define segment 1 
%                 res = inst.SendScpi(strcat(':TRAC:DEF 1,',num2str(length(chirps{1}.dacSignal)))); % define memory location 1 of length dacSignal
%                 assert(res.ErrCode == 0);
...
%                 chirps{1}.dacSignal = uint8(chirps{1}.dacSignal);
%                 res = inst.WriteBinaryData(':TRAC:DATA 0,', chirps{1}.dacSignal);
%                 assert(res.ErrCode == 0);
% %                 srs_freq_str = [':SOUR:NCO:CFR1 ' sprintf('%0.2e',  sampleRateDAC - )]; %srs_freq
...
%                 use the following to manually trigger the task
...
%               for the test (off)
    ...
    for i = 1:2
        ...
        rc = inst.SendScpi(':SOUR:MODE DUC');
            assert(rc.ErrCode == 0);
        %rc = inst.SendScpi('SOUR:INT X4');
        %assert(rc.ErrCode == 0);
        %rc = inst.SendScpi('SOUR:IQM ONE');
        %assert(rc.ErrCode == 0);
        
        if strcmp(test,'off') == 1
%             srs_freq_str = [':SOUR:NCO:CFR1 ' sprintf('%0.2e',  sampleRateDAC - chirps{i}.srs_freq)]; %srs_freq
            srs_freq_str = [':SOUR:NCO:CFR1 ' sprintf('%0.2e', chirps{i}.srs_freq)]; %srs_freq
            res = inst.SendScpi(srs_freq_str);
            assert(res.ErrCode == 0);
           ...
           inst.SendScpi(':NCO:SIXD1 ON');
            ...
            try
                ...
                sampleRateDAC_str = [':FREQ:RAST ' sprintf('%0.2e', sampleRateDAC)];
                res = inst.SendScpi(i_str); % set sample clock
                assert(res.ErrCode == 0);
            end
    end
