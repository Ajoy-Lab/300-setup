...
%                 assert(rc == 0);
                 ...
                 rc = inst.SendScpi(sprintf(':DIG:ACQ:FRAM:DEF %d, %d',numberOfPulses_total*loops, 2 * readLen));
                 assert(rc.ErrCode == 0);
...
%                 rc = inst.ReadAdcCaptureStatus();
                ...
                netArray = NET.createArray('System.UInt16', 4*readLen*numberOfPulses_total); %total array -- all memory needed
...
%                     
                    ...
                    rc = inst.ReadMultipleAdcFrames(chanIndex, firstIndex, numberOfPulses_total, netArray); %this is where the device reads
...
%                     assert(rc == 0);
                    ...
                    pulses = reshape(samples, [], numberOfPulses_total); % reshape samples into a more useful array (2 dimensions)
...
% %                         pulse(1:41800)=[];
                        ...
                        phase=phase1-phase2;
                        %             linFFT_vec(:,i)=linFFT;
                        idx = i+(numberOfPulses_total*(n-1));
                        pulseAmp(idx) = amp;
                        relPhase(idx) = phase1;
                        %relPhase(idx) = phase2;
                    end
