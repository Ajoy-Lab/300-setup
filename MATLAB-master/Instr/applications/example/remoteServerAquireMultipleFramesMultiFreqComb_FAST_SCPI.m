...
%                 assert(rc == 0);
            ...
            case 6 % Program MW chirp waveform
                ...
                awg_center_freq = cmdBytes(2); %Hz %upconversion frequency
                awg_bw_freq = cmdBytes(3); %Hz %bandwidth for the MW chirp
                awg_amp = cmdBytes(4)/combno;
                sweep_freq = cmdBytes(5);  %the frequency of the one cycle of the chirp
...
%                 disp(['fstart = ' num2str(fStart)]);
                ...
                dacSignal = makeChirp(sampleRateDAC, rampTime, dt, fStart(i), fStop(i), bits);   
                end
