...
%                 tacq=96;
                ...
                numberOfPulses = floor(numberOfPulses_total/Tmax);
                numberOfPulses2 =floor(numberOfPulses_total2/Tmax2); %in 1 second %will be 1 for FID
                loops=Tmax;
                loops2=Tmax2;
...
%                 readLen = 97920; % for tacq=96
                ...
                netArray2 = NET.createArray('System.UInt16', readLen*numberOfPulses2); %total array -- all memory needed
                
                rc = inst.SetAdcAcquisitionEn(on,off);
                assert(rc == 0);
                
                rc = inst.SetAdcFramesLayout(numberOfPulses*loops+numberOfPulses2*loops2, readLen); %set memory of the AWG
                assert(rc == 0);
...
%                     end
                ...
                for n = loops+1:loops+loops2
                    fprintf('Start Read %d .... ', n);
                    firstIndex = ((n-1-loops)*numberOfPulses2+loops*numberOfPulses)+1;
                    tic
                    rc = inst.ReadMultipleAdcFrames(chanIndex, firstIndex, numberOfPulses2, netArray2); %this is where the device reads
                    assert(rc == 0);
...
%                     end
                ...
                time_cycle2=85e-6+delay2+(tacq+2+4+2)*1e-6 %for square
