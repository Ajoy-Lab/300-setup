...
%     assert(res.ErrCode == 0);
    ...
    inst.SendScpi(':DIG:DDC:DEC X16');
    inst.SendScpi(':DIG:DDC:CFR1 0.0');
    inst.SendScpi(':DIG:DDC:PHAS 0.0');
...
%                 tacq=96;
                ...
                loops=Tmax;
                 ...
                 readLen = round2((tacq+2)*1e-6 * 0.125/1e-9,96)-96;
...
%                 assert(rc==0);
                ...
                %
                rc = inst.SendScpi(sprintf(':DIG:TRIG:LEV1 %g', 0.3));
                assert(rc.ErrCode == 0);
...
%                 rc = inst.ReadAdcCaptureStatus();
                ...
                for n = 1:loops
                    ...
                    netArray2 = NET.createArray('System.UInt16', wavlen);
                    nFrames = 100;
%                     netArray2 = NET.createArray('System.UInt16', nFrames * readLen);
                    
                    tic
                                                      
                    rc = inst.ReadMultipleAdcFrames(chanIndex, firstIndex, numberOfPulses, netArray2); %this is where the device reads
                    %rc = inst.ReadMultipleAdcFrames(0, firstIndex, nFrames, netArray2); %this is where the device reads
...
%                 rc = inst.SetAdcCaptureEnable(off);
                ...
                rc = inst.SendScpi(':DIG:ACQ:FRAM:FREE');
                assert(rc.ErrCode == 0);
