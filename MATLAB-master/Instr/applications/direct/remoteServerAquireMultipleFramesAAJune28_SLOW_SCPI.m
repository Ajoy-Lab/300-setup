...
%                 numberOfPulses_total=169993;
                ...
                %pw=round(190*10^((37-cmdBytes(5))/20))*1e-6;
                tacq=cmdBytes(5);
%                 tacq=128;
%                 tacq=64;
%                 tacq=96;
...
%                 assert(rc == 0);
                ...
                fprintf('Instr setup complete and ready to acquire\n');
...
%                 rc = inst.ReadAdcCaptureStatus();
                ...
                %netArray = NET.createArray('System.UInt16', readLen*numberOfPulses); %total array -- all memory needed
                netArray = NET.createArray('System.UInt16', 1e9*5e-3); %total array -- all memory needed
...
% %                         pulse(1:41800)=[];
                        ...
                        pulseAmp(idx) = amp;
                        relPhase(idx) = phase2; %positive phase
...
%                 assert(res.ErrCode == 0);
            ...
            case 10 % Program MW chirp waveform
                % ---------------------------------------------------------------------
                % Download chirp waveform of 1024 points to segment 1 of channel 1
                % ---------------------------------------------------------------------
