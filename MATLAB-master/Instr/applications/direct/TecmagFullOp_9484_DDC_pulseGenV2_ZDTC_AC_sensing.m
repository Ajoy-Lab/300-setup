...
%     assert(res.ErrCode == 0);
   ...
   18
    ...
    % Y-pulse spacing
    T = lengths(4) + spacings(4);
    
    % set PB parameter
    AC_phase_start_time = lengths(1) + spacings(1) + ...
               (lengths(2) + spacings(2))*reps(2) + ...
               lengths(3) + spacings(3) + lengths(4)/2;
...
%                 tof = -1000*cmdBytes(2);
                ...
                defPulse('init_DTC', amps(3), mods(3), lengths(3), phases(3), spacings(3));
                defPulse('gamma', amps(4), mods(4), lengths(4), phases(4), spacings(4));
                defPulse('init_DTC_read', amps(5), mods(5), lengths(5), phases(5), spacings(5));
                defPulse('theta2', amps(6), mods(6), lengths(6), phases(6), spacings(6));
                defBlock('pulsed_SL', {'init_pul','theta1'}, reps(1:2), markers(1:2), trigs(1:2));
                defBlock('DTC', {'init_DTC','gamma'}, reps(3:4), markers(3:4), trigs(3:4));
                defBlock('read_DTC_SL', {'init_DTC_read','theta2'}, reps(5:6), markers(5:6), trigs(5:6));
                makeBlocks({'pulsed_SL','DTC','read_DTC_SL'}, ch, repeatSeq);
...
%                     end
                ...
                time_axis1 = (1:reps(2))*(lengths(2)+spacings(2));
                DTC_start_time = reps(2)*(lengths(2)+spacings(2))+lengths(3)+spacings(3);
                time_axis2 = DTC_start_time + (1:reps(4))*(lengths(4)+spacings(4));
                SL2_start_time = reps(2)*(lengths(2)+spacings(2))+lengths(3)+spacings(3)+... 
