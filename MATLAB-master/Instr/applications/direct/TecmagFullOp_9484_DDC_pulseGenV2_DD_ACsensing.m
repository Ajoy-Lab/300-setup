...
%     assert(res.ErrCode == 0);
    ...
    spacing = 100e-6+pi;
    min_spacing = 0.05e-6;%(idx+1)*0.2e-6;
...
%     trigsDD = cat(2,trigsDD,trigsDDback);
    ...
    mods = {zeros(1,length(lengths{1})),zeros(1,length(lengths{2}))};
...
%                 tof = -1000*cmdBytes(2);
                ...
                defBlock('next', nextSeqNames, reps{2}, markers{2}, trigs{2});
...
%                 numberOfPulses_total = reps(2);
                numberOfPulses_total = reps{1}(2) + sum(trigs{2})*repeatSeq(2);
...
%                     plot_labels('Time [s]', 'Signal [au]');
                ...
                save(['Z:\' fn],'pulseAmp','time_axis','relPhase','AC_dict','lengths',...
                    'phases','spacings','reps','trigs','amps','repeatSeq','start_time');
