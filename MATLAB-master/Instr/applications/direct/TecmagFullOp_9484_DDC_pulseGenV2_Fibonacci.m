...
%     pulse_name = ['init_pul', 'theta1'];
    ...
    % tasktable
    memo = struct();
    [fib_seq , memo] = Fibonacci(unitary_num, memo);
    DTC_rep_seq = length(fib_seq);
    fprintf("This is length of the sequence: %d \n", DTC_rep_seq);
...
%                 tof = -1000*cmdBytes(2);
                ...
                generate_RND_DTC_PulseSeqIQ(ch, amps, frequencies, lengths, phases, spacings, markers, trigs, reps, fib_seq, num_x_lt_pulses);
...
function generate_RND_DTC_PulseSeqIQ(ch, amps, frequencies, lengths, phases, spacings, markers1, trigs, reps, fib_seq, num_x_lt_pulses)
%{
...
%         %assert(res.ErrCode==0);
    ...
    function setTask_Pulse(ch, numPulses, numSegs, reps, fib_seq, num_x_lt_pulses)
    %{
    4 types of segment:
    ...
    % set tasktable length = 2*length(fib_seq)+4
    ...
    % the task table.
    task_table_length = 3*length(fib_seq) + 4;
    ...
    for rand_idx = 1: length(fib_seq)
        rand_num = fib_seq(rand_idx);
        num_x_pulse = 0; 
        if rand_num == 0
            num_x_pulse = num_x_lt_pulses;
        elseif rand_num == 1
    ...
    setTask_Pulse(ch, numPulses, numSegs, reps, fib_seq, num_x_lt_pulses);
