    ...
    while ( connect==on )
        ...
        switch cmdByte
            ...
            case 2 % Aquire on trig
                ...
                for ind=1:rep
                    netArray{ind} = NET.createArray('System.UInt16', readLen*numberOfPulses); %total array -- all memory needed
                    netArray2{ind} = NET.createArray('System.UInt16', readLen*numberOfPulses2); %total array -- all memory needed
                end
                
                netArray3 = NET.createArray('System.UInt16', readLen*numberOfPulses3); %all remaining data after the reps
            ...
            case 3 % Measure
                ...
                for ind=1:rep
                    x=0;
                    for n = (ind-1)*(loops+loops2)+1:loops+(ind-1)*(loops+loops2)
                        %x=n-(ind-1)*(loops+loops2)-1;
                        x=x+1;
                        fprintf('Start Read %d .... ', n);
                        %firstIndex = ((n-1)*numberOfPulses)+1; %works when rep=1
                        firstIndex = (ind-1)*(loops*numberOfPulses+loops2*numberOfPulses2)+(x-1)*numberOfPulses+1;
                        tic
                        rc = inst.ReadMultipleAdcFrames(chanIndex, firstIndex, numberOfPulses, netArray{ind}); %this is where the device reads
                        assert(rc == 0);
                        samples = double(netArray{ind}); %get the data (1s chunk)
                        ...
                        rc =inst.WipeMultipleAdcFrames(chanIndex, firstIndex, numberOfPulses, 0);
                        assert(rc == 0);
                    ...
                    x=0;
                    for n = (ind-1)*(loops+loops2)+loops+1:(ind)*(loops+loops2)
                        fprintf('Start Read %d .... ', n);
                        %x=n-1-(ind-1)*(loops+loops2)-loops;
                        x=x+1;
                        %firstIndex =((n-1-loops)*numberOfPulses2+loops*numberOfPulses)+1; %works when rep=1
                        firstIndex = (ind-1)*(loops*numberOfPulses+loops2*numberOfPulses2)+loops*numberOfPulses+(x-1)*numberOfPulses2+1;
                        tic
                        rc = inst.ReadMultipleAdcFrames(chanIndex, firstIndex, numberOfPulses2, netArray2{ind}); %this is where the device reads
                        assert(rc == 0);
                        samples = double(netArray2{ind}); %get the data (1s chunk)
                        fprintf('Read %d Complete\n', n);
                        toc
                        ...
                        %delete mem
                        fprintf('Clear mem %d .... ', n);
                        rc =inst.WipeMultipleAdcFrames(chanIndex, firstIndex, numberOfPulses2, 0);
                        assert(rc == 0);
                        fprintf('Clear mem %d Complete\n', n);
                        toc
                        ...
                        tic
                        fprintf('Starting iteration %d data processing....', n);
                        pulses = reshape(samples, [], numberOfPulses2); % reshape samples into a more useful array (2 dimensions)
                            
                        clear samples;
                        for i = 1:numberOfPulses2
                            pulse = pulses(:, i);
                            ...
                            phase=phase1-phase2;
                            %             linFFT_vec(:,i)=linFFT;
                            pulseAmp(i+(numberOfPulses2*(n-1-loops)+loops*numberOfPulses)) = amp;
                            relPhase(i+(numberOfPulses2*(n-1-loops)+loops*numberOfPulses)) = phase1;
                        end
