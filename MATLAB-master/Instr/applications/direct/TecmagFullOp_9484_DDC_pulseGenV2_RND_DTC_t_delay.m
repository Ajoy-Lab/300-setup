...
%     pulse_name = ['init_pul', 'theta1'];
    ...
    trigs = [0 1 1 1 1];
    markers = [1 1 1 1 1]; %always keep these on => turns on the amplifier for the pulse sequence
    reps = [1 5999 1 1 300];
    ...
    assert(num_x_lt_pulses < reps(5), "number of x-pulses applied, left of the Y pulse should be less than all x pulses in a block");
...
%                     end
                ...
                for i = (1:length(random_seq))
                    ...
                    elseif rand_num == 1
                        num_x_pulse = reps(5) - num_x_lt_pulses;
                    else
                        assert((rand_num == 0 | rand_num == 1), "rand_num should be 0 or 1"); 
                    end
                    %add time for the x pulses left of the Y pulse
                    time_axis_added = curr_t + (1:num_x_pulse)*(lengths(5)+spacings(5));
                    time_axis = cat(2, time_axis, time_axis_added);
                    curr_t = curr_t + num_x_pulse*(lengths(5) + spacings(5));
                    %add time for the Y pulse
                    curr_t = curr_t + lengths(4)+spacings(4);
                    time_axis = [time_axis, curr_t];
                    %add time for the x pulses right of the Y pulse
                    time_axis_added = curr_t + (1:(reps(5)-num_x_pulse))*(lengths(5)+spacings(5));
                    time_axis = cat(2, time_axis, time_axis_added);
                    curr_t = curr_t + (reps(5)-num_x_pulse)*(lengths(5) + spacings(5));
                end
...
%         %assert(res.ErrCode==0);
    ...
    % +1 comes from setting the last train of x-pulse with long delay time.
    % y-pulse and 2 trains of x-pulse (front of y-pulse and back of y-pulse) to create DTC requires 3 entries in
    % the task table.
    task_table_length = 3*length(random_seq) + 5;
    ...
    t_idx = t_idx + 1;
    inst.SendScpi(sprintf(':TASK:COMP:SEGM %d',6));
    ...
    inst.SendScpi(':TASK:COMP:TYPE SING');
     ...
     %% set one x-pulse to set t-decay
    inst.SendScpi(sprintf(':TASK:COMP:SEL %d',t_idx));
    t_idx = t_idx + 1;
    ...
    for rand_idx = 1: length(random_seq)
        ...
        t_idx = t_idx + 1;
        inst.SendScpi(sprintf(':TASK:COMP:SEGM %d', 5));
        inst.SendScpi(sprintf(':TASK:COMP:LOOP %d', reps(5) - num_x_pulse));
        inst.SendScpi(sprintf(':TASK:COMP:NEXT1 %d',t_idx));
        inst.SendScpi(':TASK:COMP:TYPE SING');   
    end
