...
%     repeatSeq = 1;
                ...
                defPulse('init_pul', 1, 0, 60e-6, 0, 100e-6);
                defPulse('theta', 1, 0, 60e-6, 90, 43e-6);
                defBlock('pulsed_SL', {'init_pul','theta'}, [1 194174], [1 1], [0 1]);
                makeBlocks({'pulsed_SL'}, 1, [1]);
...
%         %assert(res.ErrCode==0);
    ...
    for y = 1:numBlocks
        ...
        for z = 1:lenBlock
            ...
            segMat{1,x} = [tempDCi tempI];
            segMat{2,x} = [tempDCq tempQ];
            segMat{3,x} = [markDC' markIQ'];     
            x = x+1;
       end
    end
    %%% MAKE FINAL SEGMENT %%% 
