...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     inputArray = 5:5:140; %LOOK AT ME!!!!!
    for j = 1:length(inputArray)
        
    readLen = 33888; % integer div by 96 % max set by tacq or memory limit
%     readLen is 33888 for 30s or less, 16896 for 30-60s, 8448 for 60-120s
%     readLenSeconds = readLen/1e9; number of points in a frame

    framesInOneWindow=100;
...
%             phase = angle(X);
    ...
    %             linFFT_vec(:,i)=linFFT;
%             pulsePhase(i+((n-1)*framesInOneWindow)) = phase;
            pulseAmp(i+((n-1)*framesInOneWindow)) = amp; % save amplitude as one point
            
            phase1=angle(X(b1));
            phase2=angle(X(b2));
            phase=phase1-phase2;
            relPhase(i+(framesInOneWindow*(n-1))) = phase;
        end
        clear pulseWindow;
...
%         Plot the last pulse of each window
    ...
    time_cycle=30*1e-6+(4+2+2+3+32)*1e-6;
...
%     save(['Z:\' fn],'pulsePhase','pulseAmp','time_axis');
