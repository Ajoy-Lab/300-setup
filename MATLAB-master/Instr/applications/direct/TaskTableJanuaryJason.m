...
%                 netArray = NET.createArray('System.UInt16', (4*readLen*numberOfPulses); %total array -- all memory needed
                ...
                for n = 1:loops
                    ...
                    rc = inst.SendScpi(':DIG:DATA:SEL FRAM'); %change to SEL FRAM
                    assert(rc.ErrCode == 0);
                    
                    rc = inst.SendScpi(sprintf(':DIG:DATA:FRAM %d,%d', firstIndex,numberOfPulses));
                    assert(rc.ErrCode == 0);
...
%                   netArray = NET.createArray('System.UInt16', wavlen);
                    nFrames = 100;
                    %netArray = NET.createArray('System.UInt16', nFrames * wavelen);
                    %netArray = zeros(numberOfPulses,2*readLen, 'uint16');
                    netArray = zeros(numberOfPulses,2*readLen, 'uint16');
                    tic             
%                     
                    netArray = inst.ReadBinaryData(':DIG:DATA:READ?', 'uint16'); % instead of readADCMultipleFrames                                    
                    %rc = inst.ReadMultipleAdcFrames(chanIndex, firstIndex, numberOfPulses, netArray); %this is where the device reads
                    %%%2023-01-10 PROGRAM FAILS HERE 
                    assert(netArray.ErrCode == 0);
%                     samples = double(netArray2); %get the data (1s chunk)
                    samples = double(netArray.RespStr); %get the data (1s chunk)
                    samples = samples;
