...
%     pulse_name = ['init_pul', 'theta1'];
    ...
    total_seq_time = 0;
    assert(length(lengths) == length(spacings) & length(spacings) == length(reps), ...
        "length of the lengths spacings and reps must be the same");
    for i = 1 : length(lengths)
        total_seq_time = total_seq_time + (lengths(i) + spacings(i))*reps(i);
    end
...
%                 tacq=96;
                ...
                %Multiple of 96 (e.g) when tacq = 32, readLen = 5664
...
%                 resp3 = strtrim(pfunc.netStrToStr(resp3.RespStr));
            ...
            case 3 % Measure
                ...
                % total_seq_time + 50 seconds (50 second is hardcoded)
                % multiplied by 10, because n is the number of 1e-1 seconds
                total_digitizer_acq_time = (total_seq_time + 50)*10;
                for n = 1:total_digitizer_acq_time
