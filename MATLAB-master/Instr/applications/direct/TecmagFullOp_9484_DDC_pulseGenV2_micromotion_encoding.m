...
%     assert(res.ErrCode == 0);
    ...
    lengths = [pi/2 pi/2 0.96*pi pi/2];
    ...
    % applied pulses
    encoding_seq = str_to_0s_and_1s('Experimental observation of a time rondeau crystal: Temporal Disorder in Spatiotemporal Order. ');
    encoding_seq(encoding_seq == 0) = -1;
    random_seq = [];
    for i = (1:length(encoding_seq))
        m = i - 1;
        random_seq(end+1) = ((-1)^m)*encoding_seq(i);
    end
    random_seq(random_seq == -1) = 0;
    DTC_rep_seq = length(random_seq);
    num_x_lt_pulses = 100;
