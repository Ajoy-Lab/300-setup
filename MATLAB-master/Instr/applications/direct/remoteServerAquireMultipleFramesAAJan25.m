...
proteusLatency = 0.0000172; % inherent delay in our device
...
%         cmdBytes = fread(u2);
        ...
        switch cmdByte
            case 1 % Setup
                ...
                rc = inst.SetAdcExternTrigDelay(0);
                assert(rc==0); 
...
%                 pw= 40*1e-6;
                ...
                Tmax=cmdBytes(4);
                tacq=cmdBytes(5);
                numberOfPulses= floor(numberOfPulses_total/Tmax); %in one second  %will be 1 for FID
                loops=Tmax;
                
                readLen = round2(((tacq+2)*1e-6/1e-9),96)-96 %constraint: has to be multiple of 96, add 4 of deadtime
                cmdBytes
...
%                         [amp, loc] = max(linFFT);
            ...
            %             linFFT_vec(:,i)=linFFT;
                ...
                time_cycle=pw+proteusLatency+(4+2+2+tacq)*1e-6;
